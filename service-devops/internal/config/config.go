...
func Load(p string) (*Config, error) {
	...
	v.SetEnvPrefix("LEAP_DEVOPS")
	v.SetEnvKeyReplacer(strings.NewReplacer(".", "_"))
	v.AutomaticEnv()
	v.SetDefault("server.host", "0.0.0.0")
	v.SetDefault("server.port", 8007)
	...
	v.SetDefault("database.db_name", "devops_db")
	...
	if e := v.ReadInConfig(); e != nil {
		if _, ok := e.(viper.ConfigFileNotFoundError); !ok {
			return nil, fmt.Errorf("读取配置失败:%w", e)
		}
	}
	var cfg Config
	if e := v.Unmarshal(&cfg); e != nil {
		return nil, fmt.Errorf("解析配置失败:%w", e)
	}
	return &cfg, nil
}
